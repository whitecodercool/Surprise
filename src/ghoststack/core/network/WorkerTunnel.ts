...
export class WorkerTunnel {
   ...
   */
  static async establishRawTunnel(
    targetHost: string,
    targetPort: number,
    workerUrl: string
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(
          `[WorkerTunnel] 🛡️ Opening blind pipe to ${targetHost} via ${parsedUrl.hostname}...`
        )
        ...
        ws.on('unexpected-response', (_, res) =>
          reject(new Error(`Worker refused upgrade: ${res.statusCode}`))
        )
      } catch (err) {
        reject(err)
      }
   ...
   */
  static async establishTLSTunnel(
    targetHost: string,
    targetPort: number,
    workerUrl: string
