...
export function DiagnosticsLogList() {
  ...
  useEffect(() => {
    ...
    setLogs([{
      ...
      status: 'failed',
      networkInfo: { ip: '104.18.2.1', region: 'California', country: 'US', isp: 'Cloudflare, Inc.' },
      failureDiagnostics: {
        ...
        reproductionCurl: 'curl -v -H "User-Agent: Mozilla/5.0" https://restricted-site.com/login',
        tlsState: { ja3Fingerprint: 'mocked-ja3-hash', cipherSuiteUsed: 'TLS_AES_128_GCM_SHA256' },
        responseDump: { statusCode: 403, headers: { 'server': 'cloudflare' }, bodySnippet: '<!DOCTYPE html><html><body><h1>Please turn on JavaScript and reload the page.</h1>...</body></html>' },
  ...
  return (
    <div className="mt-6 flex flex-col gap-2">
      <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Diagnostic Logs</h3>
      <div className="bg-[#0a0a0f] border border-[#222] rounded-xl overflow-hidden shadow-lg">
        <table className="w-full text-left text-sm text-gray-300">
          ...
          <tbody className="divide-y divide-[#222]">
            {logs.map(log => (
              ...
              >
                <td className="px-4 py-3">
                  {log.status === 'failed' && <span className="material-symbols-outlined text-red-500 text-lg block">gpp_bad</span>}
                  {log.status === 'success' && <span className="material-symbols-outlined text-green-500 text-lg block">check_circle</span>}
                  {log.status === 'running' && <span className="material-symbols-outlined text-blue-500 text-lg block animate-spin">refresh</span>}
                </td>
                <td className="px-4 py-3 truncate max-w-[200px]" title={log.url}>{log.url}</td>
                <td className="px-4 py-3 truncate">{log.networkInfo.region || 'Unknown'}</td>
                <td className="px-4 py-3 truncate" title={log.networkInfo.isp}>{log.networkInfo.isp || 'Unknown'}</td>
            ...
            {logs.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-gray-600 italic">No network logs recorded yet.</td>
              </tr>
            )}
          </tbody>
        </table>
