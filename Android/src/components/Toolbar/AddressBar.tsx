...
export default function AddressBar() {
  ...
  // Listen for focus triggers from other components (like mobile New Tab search button)
  useEffect(() => {
    const handleFocusTrigger = () => {
      inputRef.current?.focus()
    }
    window.addEventListener('focus-address-bar', handleFocusTrigger)
    return () => window.removeEventListener('focus-address-bar', handleFocusTrigger)
  }, [])
    ...
    <div className="address-bar-container" ref={containerRef}>
      <form
        ...
        style={{
          ...
          background: isMobile ? 'var(--color-bg-secondary)' : undefined,
          borderColor: isMobile ? (isFocused ? 'var(--color-accent)' : 'var(--color-border-medium)') : undefined
        }}
      >
